...
| Shared queues names in overflow                                        | Displays a list of shared queue names that are in overflow mode for specified coupling facility list structures.                            | ims_dis_overflowq                                     |
| Shared queues by resource type                                         | Displays shared queue information for the specified resource type.                                                                          | ims_display_qcnt                                      |
| Database description information                                       | Displays information for databases that are being accessed by their type, the PSBs accessing them, and the type of access.                  | ims_dis_dbd                                           |
| DEDB area data set information                                         | Displays area data sets, status conditions, control interval counts, and databases associated with an area.                                 | ims_dis_area                                          |
| PSB information                                                        | Displays transaction codes, route codes, and databases associated with the PSB and the type of access for each listed database.             | ims_get_psb_info                                      |
| CQS status                                                             | Displays the status of the IMS Common Queue Server (CQS).                                                                                   | ims_get_cqs_info                                      |
| Trace definitions and status                                           | Displays IMS trace definitions and status.                                                                                                  | ims_get_trace_info                                    |
| Message queue status                                                   | Displays IMS message queue status with class and priority details.                                                                          | ims_get_queue_info                                    |
| OTMA transaction member and pipe status                                | Displays IMS OTMA transaction member status, transaction pipe information, and queue counts.                                                | ims_dis_tmember                                       |
| Transaction SYSID information                                          | Displays remote ID and local ID for systems associated with the specified transactions.                                                     | ims_sysid_transaction                                 |
...
Under `ims-agent` directory there is a folder named `imagesign` that contains a file named `public.pub.asc`. Place this file in a location of your choice. Then, copy the following Docker container policy `policy.json` file into the`/etc/containers/policy.json` and update the `keyPath` field to reflect the location of your `public.pub.asc`.
...
This is followed by the public key's fingerprint and the digest of the image. If a failure occurs, you might see this error:
...
If IFM-Lite is supported in the environment, do not add a space ID. For example, WATSONX_DEPLOYMENT_SPACE_ID must be an empty string.
...
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **MCP secrets**  |                                                                                                                                                                                                                                                                                              |
| ZOSMF_ENDPOINT   | The base URL for the z/OS Management Facility (z/OSMF), provided by IBM for managing and interacting with z/OS systems, for example: `https://your.zos.system.com`. It is recommended that you use a host because using an IP address might cause issues.                                                            |
| SERVICE_ENDPOINT | Defines the URL or network address where z/OSMF services are exposed. This endpoint should match the ZOSMF_ENDPOINT but use a different port where mTLS authentication is set, for example `https://your.zos.system.com:5443`. It is recommended that you use a host because using an IP address might cause issues.. |
| AGENT_AUTH_TOKEN | Authentication token for the agent.                                                                                                                                                                                                                                                          |

Additionally, you can update the `mcpCertSecret` section of the `values.yaml` file before running the helm-install command. For more information, see the section [Configuring your z/OSMF certificate](https://github.ibm.com/ims/ims-ai/blob/main/README.md#configuring-your-zosmf-certificate-for-the-mcp-container-image) for the MCP container image.
...
The CN and SAN domain (your.zos.system.com in this example) must exactly match the host name used in your `ZOSMF_ENDPOINT` environment variable.

> The previous JCL assumes the APPL ID is `IZUDFLT`. You might need to stop and restart z/OSMF for changes to take effect, such as running `/P IZUSVR1` and `/S IZUSVR1`. Modify as needed.
...
> **Tip**: If you are installing multiple agents, you can configure the [values.yaml](https://github.com/IBM/z-ai-agents/blob/main/wxa4z-agent-suite/values.yaml) file for all the agents that you want to install. After the file is updated, run the following command to install them all at the same time.
...
> You can also configure the IBM IMS Agents' NetworkPolicies.
> By default, all traffic is allowed for simplicity, which ensures automatic connectivity. If your organization requires stricter security, you can customize NetworkPolicies in the Helm charts.
...
4. Enter queries by using the AI Assistant, for example:
...
If you encounter any errors during installation, see [Troubleshooting](../../README.md#troubleshooting) for troubleshooting steps.
...
The QA Agent relies on the OpenSearch pod for retrieval-augmented generation (RAG). If the agent cannot communicate with the OpenSearch instance deployed to the target cluster, it might hallucinate (generate nonfactual text), resulting in low-quality or unhelpful responses.

#### Steps to resolve IMS QA Agent issues

1. Ensure all required environment variables are correctly set.

2. Confirm that the OpenSearch instance is deployed to the target cluster and is running.

3. Restart the pod to pick up the new changes.
...
#### Steps to resolve IMS MCP Agent issues

1. Ensure all required environment variables are correctly set.

   **Tip:** It is recommended that you use a host name instead of an IP address in the `ZOSMF_ENDPOINT` and `SERVICE_ENDPOINT` environment variables because using an IP address might cause issues.

2. Validate the `wxa4z-authorization` service. This service has two critical components:

   - **Authorization Pod**

     Check whether the `wxa4z-authorization` pod was deployed and is running on the target cluster.

   - **Token Exchanger Service**

     Confirm that the token-exchanger service is configured and running on your target z/OS system.
     If it does not exist, [deploy and start the service](https://github.ibm.com/wxa4z/tokenexchange/releases/tag/v0.1.0).

     **Tip:** You can check whether the token-exchanger is running by using SSH to log in to your z/OS system (for example, `ssh username@your.zos.system.com`) and running the following command to see whether the process is running: `ps -ef | grep java`. If you see `java -jar token-exchange-mtls.jar` in the results list, the token-exchanger is running.

     You can also check the logs from the token-exchanger: `scp username@your.zos.system.com:path/to/passticket-mtls/nohup.out ~/Download/log.txt`. This will download the logs to your local workstation and place them here: `~/Download/log.txt`

3. Check z/OSMF and Operator Console

   - Ensure z/OSMF is running and an Operator Console is set up and active.
   - Ensure the z/OSMF Certificate has been created and the corresponding secret (`service-endpoint-cert-secret`) has been updated in OpenShift.
...
4. Restart the pod to pick up new changes.
